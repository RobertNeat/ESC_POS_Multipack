...
The API address can be changed from the `API connection` card on the
`Dashboard` page and is stored in the browser.
...
The `Printer settings` page configures the device itself (firmware settings
and actions such as autotest or factory reset), not the API connection.
